module.exports = function(express, app, __dirname) {
	...
	AdminRoutes.all_merchants = function(req, res, next) {
		...
		.fail(function (err) {
			errorHandler.logAndReturn('Error getting merchant profiles from admin', 500, next, err);
		});
	};
	...
	AdminRoutes.get_available_keys = function(req, res, next) {
		...
		.fail(function (err) {
			errorHandler.logAndReturn('Error getting available keys from admin', 500, next, err);
		});
	};
	...
	AdminRoutes.get_hash = function(req, res, next) {
		...
		.fail(function (err) {
			errorHandler.logAndReturn('Error getting user hash from admin', 500, next, err);
		});
	};
